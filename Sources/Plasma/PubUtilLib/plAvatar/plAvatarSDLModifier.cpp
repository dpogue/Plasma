...
void plAvatarSDLModifier::IPutBaseAvatarStateIn(plArmatureMod *avMod, plStateDataRecord* dstState)
{
    if (avMod->GetStealthLevel() > 0)
        plNetClientMgr::GetInstance()->DebugMsg("plAvatarSDLModifier::IPutBaseAvatarStateIn - Stealth level being set greater than zero");
...
void plAvatarSDLModifier::ISetBaseAvatarStateFrom(plArmatureMod *avMod, const plStateDataRecord* srcState)
{
    ...
    if (!avMod->IsLocallyOwned())
    {
        ...
        if (invisLevel > 0)
            plNetClientMgr::GetInstance()->DebugMsg("plAvatarSDLModifier::ISetBaseAvatarStateFrom - Stealth level greater than zero");
